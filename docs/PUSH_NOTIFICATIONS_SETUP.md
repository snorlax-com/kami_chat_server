# 創設者（占い師）返信プッシュ通知（FCM）

## 概要

創設者（占い師）が占い相談チャットに返信したとき、ログイン済みユーザーの端末へ FCM プッシュを送ります。

- **タイトル**: AuraFaceから新しい導きが届きました
- **本文**: 創設者（占い師）から返信が届いています。タップして確認してください。
...
5. 創設者（占い師）返信: `POST /api/chat/dev-reply` または Gmail の返信リンク `/admin/reply`
6. 端末に通知 → タップで占い相談タブへ
